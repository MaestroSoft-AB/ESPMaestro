...
void _lv_inv_area(lv_disp_t *disp, const lv_area_t *area_p) {
  ...
  bool full_screen_inv =
      com_area.x1 == scr_area.x1 && com_area.y1 == scr_area.y1 &&
      com_area.x2 == scr_area.x2 && com_area.y2 == scr_area.y2;
  if (full_screen_inv &&
      esp_timer_get_time() > LVGL_TRACE_INVALIDATION_BOOT_SKIP_US) {
    ESP_LOGW(LVGL_TRACE_INVALIDATION_TAG,
             "LVGL_TRACE invalidate full screen req=%d,%d-%d,%d inv_p=%u "
             "full_refresh=%d direct=%d",
  ...
  if (disp->inv_p < LV_INV_BUF_SIZE) {
    ...
    if (esp_timer_get_time() > LVGL_TRACE_INVALIDATION_BOOT_SKIP_US) {
      ESP_LOGW(LVGL_TRACE_INVALIDATION_TAG,
               "LVGL_TRACE invalidate overflow: inv_p=%u max=%u "
               "fallback=%d,%d-%d,%d",
...
static void refr_sync_areas(void) {
  ...
  void *buf_on_screen =
      disp_refr->driver->draw_buf->buf_act == disp_refr->driver->draw_buf->buf1
  ...
  for (sync_area = _lv_ll_get_head(&disp_refr->sync_areas); sync_area != NULL;
    ...
    disp_refr->driver->draw_ctx->buffer_copy(disp_refr->driver->draw_ctx,
...
void refr_obj(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj) {
  ...
  if (layer_type == LV_LAYER_TYPE_NONE) {
    ...
    if (LV_COORD_IS_PCT(pivot.x)) {
      pivot.x =
          (LV_COORD_GET_PCT(pivot.x) * lv_area_get_width(&obj->coords)) / 100;
    }
    if (LV_COORD_IS_PCT(pivot.y)) {
      pivot.y =
          (LV_COORD_GET_PCT(pivot.y) * lv_area_get_height(&obj->coords)) / 100;
    }
