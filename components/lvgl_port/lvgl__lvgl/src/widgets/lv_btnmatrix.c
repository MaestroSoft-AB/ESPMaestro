...
void lv_btnmatrix_set_btn_ctrl(lv_obj_t *obj, uint16_t btn_id,
  ...
  /* If we hide a button if all buttons are now hidden hide the whole button
   * matrix to make focus behave correctly */
...
void lv_btnmatrix_clear_btn_ctrl(lv_obj_t *obj, uint16_t btn_id,
  ...
  /* If all buttons were hidden the whole button matrix is hidden so we need to
   * check and remove hidden flag if present */
  if (ctrl & LV_BTNMATRIX_CTRL_HIDDEN) {
