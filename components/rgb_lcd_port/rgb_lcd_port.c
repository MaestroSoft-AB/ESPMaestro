...
esp_lcd_panel_handle_t waveshare_esp32_s3_rgb_lcd_init() {
  ...
  esp_lcd_rgb_panel_config_t panel_config = {
      ...
      .timings =
          {
              .pclk_hz =
                  EXAMPLE_LCD_PIXEL_CLOCK_HZ, // Pixel clock frequency in Hz
              .h_res = EXAMPLE_LCD_H_RES, // Horizontal resolution (number of
                                          // pixels per row)
              .v_res =
                  EXAMPLE_LCD_V_RES,    // Vertical resolution (number of rows)
              ...
              .flags =
                  {
                      .pclk_active_neg =
                          1, // Set pixel clock polarity to active low
                  },
          },
